<!-- <template>
    <div class="create-post">
      ...
      <div class="container">
        ...
        <div class="camp-actions">
          <button @click="updateCamp">Save Changes</button>
          <router-link class="router-button" :to="{ name: 'CampPreview' }">Preview Changes</router-link>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    async mounted() {
      this.routeID = this.$route.params.campid;
      this.currentCamp = await this.$store.state.campPosts.filter((post) => {
        return post.campID === this.routeID;
      });
      this.$store.commit("setCampState", this.currentCamp[0]);
    },
    methods: {
      fileChange() {
      ...
      async updateCamp() {
        const dataBase = await db.collection("campPosts").doc(this.routeID);
